...
class Product extends Component {
	...
	displayImages = () => {
		...
		return (
			<Fragment>
				<img
					className="pic-1"
					src={product.images[0] ? product.images[0].url : defaultImg}
				/>
				<img
					className="pic-2"
					src={product.images[1] ? product.images[1].url : defaultImg}
				/>
			</Fragment>
		);
	};
	...
	render() {
		return <Fragment>{this.renderProduct()}</Fragment>;
	}
}
